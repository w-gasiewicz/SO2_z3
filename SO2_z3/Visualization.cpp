...
void Visualization::Legend(){
    ...
    attron(COLOR_PAIR(6));
    mvprintw(5,1,"Sales:");
    mvprintw(6,1,"Serviced clients:");
    mvprintw(7,1,"Cash earned:");
...
void Visualization :: Start(Client clients[],Distributor distributors[], SalesMan salesmans[], vector<int> & gasolineQueue,vector<int> & paymentQueue, vector<int> & washQueue, CarWash & carwash, GasStation & station){//function start visualization
    int inputChar;
    ...
    while(true){
        ...
        if(inputChar!=113){//while user dont press q button
            ...
            return;    
        attron(COLOR_PAIR(5));
        mvprintw(6,18,"%d",station.servicedCustomers);
        mvprintw(7,18,"%.2f",station.income);
        //######################################GASOLINE MAINTANCE########################################################################
        int ycord=21;        
        ...
        int clientsIDs[8];
            for(int i=0;i<8;i++){
                ...
                /*if(distributors[i].isUsed && clientsIDs[i]!=distributors[i].clientID){
                 ...
                 else */if(distributors[i].isUsed /*&& clients[distributors[i].clientID].id==distributors[i].clientID*/){
